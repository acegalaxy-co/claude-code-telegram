...
export async function startBridge({
  ...
  // Brute-force tracker: chatId → { count, firstAt, alertedAt }.
  // Threshold: 5 denies within 5 min triggers a DM to admins (rate-limited 15min).
  const denyTracker = new Map();
  const DENY_THRESHOLD = 5;
  const DENY_WINDOW_MS = 5 * 60 * 1000;
  const DENY_ALERT_COOLDOWN_MS = 15 * 60 * 1000;
  async function trackDeny(chatId, userId, kind, reason) {
    const now = Date.now();
    let t = denyTracker.get(chatId);
    if (!t || now - t.firstAt > DENY_WINDOW_MS) {
      t = { count: 0, firstAt: now, alertedAt: 0 };
      denyTracker.set(chatId, t);
    }
    t.count += 1;
    if (t.count >= DENY_THRESHOLD && (now - t.alertedAt) > DENY_ALERT_COOLDOWN_MS && access.admins) {
      t.alertedAt = now;
      audit.write("alert.bruteforce", { chatId, userId, count: t.count, kind, reason });
      const text = `🚨 Brute-force suspected\nchat=${chatId} user=${userId ?? "?"}\n${t.count} denies in ${Math.round((now - t.firstAt) / 1000)}s\nlast: ${kind} (${reason})`;
      for (const adminId of access.admins) {
        try { await tg.sendMessage(adminId, text); }
        catch (e) { logger.warn(`bruteforce alert DM failed to ${adminId}: ${e.message}`); }
      }
    }
  }
    ...
    for (const upd of updates) {
      ...
      try {
        ...
        if (!accessCheck.ok) {
          ...
          trackDeny(chatId, fromUser.id ?? null, accessCheck.kind, accessCheck.reason).catch(() => {});
          continue;
        }
...
async function handleCommand(tg, s, chatId, msg, { projectsRoot, defaultProject, fallbackCwd, logger, store, me, startedAt, audit, access }) {
  ...
  switch (cmd) {
    ...
    case "/reset": {
      ...
      const prevSid = getSessionId(s);
      clearSessionId(s);
      persist(store, chatId, s);
      audit?.write("session.reset", {
        chatId, userId: msg.from?.id ?? null,
        cli: s.cli, project: s.project || null,
        prevSessionId: prevSid ? prevSid.slice(0, 8) : null,
      });
    ...
    case "/audit": {
      ...
      // Truncate at line boundary, then HTML-escape AFTER truncation so we
      // never split an HTML entity (which would break Telegram render).
      const MAX = 3800;
      let total = 0;
      const kept = [];
      ...
      const body = kept.join("\n") + (dropped > 0 ? `\n… (${dropped} earlier event(s) elided)` : "");
